...
if(len(acc)<=2000):
    ...
    plt.savefig(f"./Molecular_Weight_output/molecular_weight.pdf",bbox_inches="tight")
   
else:
    with open("./Molecular_Weight_output/Molecular_weight.tsv","w") as wh:
        wh.write("Accession_ID\tAA_length\tMol_Wt\n")
        for ac in range(len(acc)):
            plen=len(protein_list[ac])
            add=0
            for b in range(0,plen):
...
print("The outputs are saved in 'Molecular_Weight_output' folder")
