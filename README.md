...
- The **Structure Prediction** tool requires a protein sequence database to operate. You can use the default uniprot reviewed database **uniprot_rev.fasta.gz** or you can also provide any other protein sequence database from Uniprot in **.fasta** or **.fasta.gz** format. Ensure that the database file is placed in the **datasets** folder for proper execution. 
